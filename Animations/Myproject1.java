...
public class Myproject1  implements ActionListener,Runnable,ListSelectionListener, ItemListener
{
    JFrame window;
	...
	JScrollPane jscroll;
	JList lst;
    JButton prev, next,b,stop,addP,removeP,saveP;
    JFileChooser fc;
    JMenuBar m;
    ...
    ImageIcon i1;
    static int  NOTSTARTED = 0, PLAYING = 1,PAUSED = 2,FINISHED = 3,PLAYING2=0;
    Player player;
	...
	void newPlaylist()
	{
		String str1=JOptionPane.showInputDialog(null,"Enter Name of New PlayList");
	...
	void initChoice()
	{
		String s[]=listname.allname();
		for(int i=0;i<listname.listnm.size();i++)
		{
				choice.add(s[i]);
		}
    ...
    {  
		try
		{
			...
			else if(e.getSource()==prev)
			{
				...
				name=(String)listmodel.getElementAt(i);
				try
						{
							fi=new FileInputStream(name);
							initi(fi);
...
class ListName
{
	...
	{
		PlayList p=null;
		String s[]=new String[listnm.size()];
		for(int i=0;i<listnm.size();i++)
		{
			p=(PlayList)listnm.get(i);
			s[i]=p.str;
